...
export default class Calendar extends UIComponent {
  static defaultProps = {
    ...
    theme: 'default',
    rowFixed: false,
    maxDate: new Date(new Date().getFullYear() + 5, new Date().getMonth()),
    minDate: new Date(new Date().getFullYear() - 5, new Date().getMonth()),
  }
  ...
  constructor(props, context) {
    ...
    const selectedDate = 'selectedDate' in props ? props.selectedDate : props.defaultSelectedDate;
    this.state = {
      selectedDate,
  ...
  handleMinAndManDate(props) {
    ...
    if (maxDate) {
      const tmp = new Date(maxDate.getFullYear(), maxDate.getMonth(), maxDate.getDate() + 1); // 下一天的零点
      maxDate = new Date(tmp.getTime() - 1); // 减一毫秒，成为当前的最后一个时刻点
    }

    try {
      if (minDate && maxDate && minDate >= maxDate) {
        throw new Error('日历可选最小日期应小于可选最大日期');
      } else {
  ...
  render() {
    ...
    const { weekdays, startFromMonday, rowFixed } = this.props;
    const { style } = this;
