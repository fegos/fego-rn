...
class TestView extends Component {
          ...
          <Button
            ...
            onPress={() => {
              ...
              Fetch.all([getUserName(), getNickName()])
                .then(Fetch.spread((username, nickname) => {
                  that.setState({
                    fetchMsg: '成功',
                    list: [
  ...
  __renderContent(section, index) {
    const content = this.__renderTest(index, section.content);
    return (
  ...
  render() {
    return (
      <View style={Style.container}>
        <View style={{
          flex: 1, padding: 10, borderWidth: 5, borderColor: '#123123',
        }}
        >
          <ScrollView>
            <Accordion
        ...
        <View style={{
          flex: 0.2, flexDirection: 'row', padding: 10, borderWidth: 5, borderColor: '#123123', alignItems: 'center',
        }}
        >
          <View style={{ flex: 4 }}>
          ...
          <View style={{ flex: 6 }}>
            <ScrollView>
              {this.state.list.map((item, i) => <Text style={Style.text} key={item.id}>数据项{i + 1}:{item.name} {item.desc}</Text>)}
            </ScrollView>
          </View>
        </View>
      </View>
